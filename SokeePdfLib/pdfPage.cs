...
namespace SokeePdfLib
{
    public class pdfPage
    {
        ...
        public void addImage(BitmapImage bitmapImage, byte[] rawData, int X, int Y)
        {
            try
            {
                imageElement objImage = new imageElement(bitmapImage, rawData, X, Y);
                _elements.Add(objImage);
                objImage = null;
            }
            /**
            ...
            **/
            ...
            {
                Debug.WriteLine("exception on addImage " + ex.Message);
            }
        }
#endif

#if PARAMETER_PASSING
        public async void addImage(IRandomAccessStream imageStraem)
        {
            IRandomAccessStream resizedStream = await SokeeMagic.ImageToScaledStreamAsync(imageStraem, _width, _height);
            ...
            await jpegStream.ReadAsync(jpegImageBytes.AsBuffer(), (uint)jpegImageSize, InputStreamOptions.None);

            imageElement objImage = new imageElement(jpegImageBytes, 10, 10, _width, _height);
            _elements.Add(objImage);
            objImage = null;
        ...
        public async void addImage(IRandomAccessStream jpegStream)
        {
            ...
            imageElement objImage = new imageElement(jpegImageBytes, centerX, centerY, imageW, imageH);
            _elements.Add(objImage);
            objImage = null;
        }
#endif
        ...
        ***********/
        ...
        internal void addFonts(List<PdfFont> fonts)
        {
